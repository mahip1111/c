...
// what are the library fxn. in c:-
// In C programming, library functions are built-in functions provided by the C standard library that help perform common tasks like input/output, string handling, mathematical computations, memory management, etc. These functions are defined in header files (like stdio.h, string.h, math.h, etc.) and save time by providing pre-written, tested code. Exapmle of such fxn's are:- printf() and scanf() and so on.
...
// NOTE:- CONSTANT AND VARIABLES IN C
